...
class LoadTest < BaseEngineTest
  ...
  def test_the_keyword
    assert_equal 'load', GlooLang::Verbs::Load.keyword
  end

  def test_the_keyword_shortcut
    assert_equal '<', GlooLang::Verbs::Load.keyword_shortcut
  end
  ...
  def test_load_without_expression
    ...
    assert @engine.error?
    assert_equal GlooLang::Verbs::Load::MISSING_EXPR_ERR, @engine.heap.error.value
  end
