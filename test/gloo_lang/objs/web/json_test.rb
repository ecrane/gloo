...
class JsonTest < BaseEngineTest
  ...
  def test_the_typename
    assert_equal 'json', GlooLang::Objs::Json.typename
  end

  def test_the_short_typename
    assert_equal 'json', GlooLang::Objs::Json.short_typename
  end
  ...
  def test_setting_the_value
    o = GlooLang::Objs::Json.new @engine
    o.set_value( '{"title":"JSON data"}' )
    assert_equal '{"title":"JSON data"}', o.value
  end

  def test_messages
    msgs = GlooLang::Objs::Json.messages
    assert msgs
