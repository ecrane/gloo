...
class DateTest < BaseEngineTest
  ...
  def test_the_typename
    assert_equal 'date', GlooLang::Objs::Date.typename
  end

  def test_the_short_typename
    assert_equal 'date', GlooLang::Objs::Date.short_typename
  end
  ...
  def test_messages
    msgs = GlooLang::Objs::Date.messages
    assert msgs
    assert msgs.include?( 'now' )
  end

  def test_adds_children_on_create
    o = GlooLang::Objs::Date.new @engine
