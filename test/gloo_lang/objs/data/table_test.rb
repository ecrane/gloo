...
class TableTest < BaseEngineTest
  ...
  def test_the_typename
    assert_equal 'table', GlooLang::Objs::Table.typename
  end

  def test_the_short_typename
    assert_equal 'tbl', GlooLang::Objs::Table.short_typename
  end
  ...
  def test_messages
    msgs = GlooLang::Objs::Table.messages
    assert msgs
    assert msgs.include?( 'show' )
  end

  def test_adds_children_on_create
    o = GlooLang::Objs::Table.new @engine
