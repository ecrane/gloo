...
class GlooSystemTest < BaseEngineTest
  ...
  def test_the_typename
    assert_equal 'gloo', GlooLang::Core::GlooSystem.typename
  end

  def test_the_short_typename
    assert_equal '$', GlooLang::Core::GlooSystem.short_typename
  end

  def test_marked_as_cannot_be_created
    refute GlooLang::Core::GlooSystem.can_create?
    assert GlooLang::Objs::Script.can_create?
    assert GlooLang::Objs::String.can_create?
  end
  ...
  def test_param
    pn = GlooLang::Core::Pn.new( @engine, '$.hostname' )
    o = GlooLang::Core::GlooSystem.new( @engine, pn )
    assert o
    assert o.pn
    assert_equal 'hostname', o.param

    pn = GlooLang::Core::Pn.new( @engine, '$.user.home' )
    o = GlooLang::Core::GlooSystem.new( @engine, pn )
    assert o
    assert o.pn
